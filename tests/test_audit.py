...
class AuditTests(unittest.TestCase):
    """Audit trail: redaction, store chain, deviations, middleware.

    Converted from pytest style (tmp_path fixture) to stdlib unittest;
    behavior unchanged.
    """
    ...
    def test_redaction_masks_keys_and_tokens(self):
        obj = {
    ...
    def test_redact_string_url_credentials(self):
    ...
    def test_store_append_query_and_chain(self):
    ...
    def test_store_tamper_detection(self):
        tmp_path = self.tmp_path
        store = _store(tmp_path)
    ...
    def test_store_deviation_records(self):
        tmp_path = self.tmp_path
        store = _store(tmp_path)
    ...
    def test_agent_history_and_tool_usage(self):
        tmp_path = self.tmp_path
        store = _store(tmp_path)
        for i in range(5):
    ...
    def test_deviation_detector_flags_novel_tool(self):
        tmp_path = self.tmp_path
        store = _store(tmp_path)
    ...
    def test_deviation_detector_flags_novel_sequence(self):
        tmp_path = self.tmp_path
        store = _store(tmp_path)
        for i in range(30):
    ...
    def test_deviation_detector_flags_unseen_path(self):
        tmp_path = self.tmp_path
        store = _store(tmp_path)
    ...
    def test_deviation_detector_requires_history(self):
        tmp_path = self.tmp_path
        store = _store(tmp_path)
    ...
    def test_policy_engine_and_permission_tracker(self):
        tmp_path = self.tmp_path
        class FakeRuleStore:
    ...
    def test_audit_tool_decorator(self):
        tmp_path = self.tmp_path
        async def main():
            store = _store(tmp_path)
    ...
    def test_audited_session_wraps_calls(self):
        tmp_path = self.tmp_path
        class FakeTool:
            name = "do_thing"
            description = "fake"
    ...
    def test_backend_emit_tool_audit(self):
        tmp_path = self.tmp_path
    ...
    def test_audit_router_endpoints(self):
        tmp_path = self.tmp_path
        import os
        import uuid
        ...
        def _delete_test_project(app, name):
            # backend.paths is frozen at import, so the project below lands
            # in the real volume — remove it explicitly (see test_kernel_router).
            try:
    ...
    def test_deviation_scan_is_incremental(self):
        tmp_path = self.tmp_path
    ...
    def test_ulid_monotonic_and_sorted(self):
        import time
    ...
    def test_coordinator_turn_emits_audit_events(self):
        tmp_path = self.tmp_path
